...
try {
  var qs = new URLSearchParams(window.location.search).get('route');
  if(qs && groups.some(function(g){ return g.id === qs; })) sel = qs;
} catch(e){}
...
function sync(){
  ...
  try {
    ...
    if(sel) url.searchParams.set('route', sel); else url.searchParams.delete('route');
